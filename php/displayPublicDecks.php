...
if ($result->num_rows == 0) { // If there are no public decks
	...
	while($row = $result->fetch_assoc()) { // Go through each row and fetch deck data
		...
		//Query username of associated userID (This should be done through a table join in the previous query, ideally)
		...
		$username = $usernameRow['username']; //Retrieve the userID

		$deck = new Deck($deckID, $title, $description, $imageBlob, $userID, 1);
		$deck->displayPublicDeck($username); //Echo HTML of Deck
	}
}
